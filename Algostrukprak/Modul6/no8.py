...
class LinkedList:
    ...
    def mergeSort(self):
        ...
        # Option 1 (passing Node directly):
        left = self.mergeSort(middle.pointer)
        right = self.mergeSort(middle.pointer)

        # Option 2 (creating a sub-list):
        # sub_list = LinkedList()
        # sub_list.head = middle.next
        # left = self.mergeSort(sub_list)
    ...
    def merge(self, left, right):
        merged = tail = Node()  # Initialize a new merged list
        ...
        tail.pointer = left if left else right  # Append remaining elements
        return merged.pointer  # Return the head of the merged list
...
sorted_list = ll.mergeSort()

print("\nSorted List:")
print_list(sorted_list)
for i in range(0, len(a)):
    advLL.AtBegining(a[i])
    advLL.AtEnd(a[i])
